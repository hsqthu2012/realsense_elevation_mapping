    ...
    float *point_var)
{
  ...
  // ROS_INFO("check1"); // added by Suqin He for debugging
  ...
  // ROS_INFO("check2"); // added by Suqin He for debugging
	return true;
}
    ...
    float *point_var)
{
  ...
  tf::StampedTransform transformTf;
  try {
    // ROS_INFO("targetFrame = %s, inputFrameId = %s,", targetFrame.c_str(), inputFrameId.c_str()); // add by Suqin He for debugging
  ...
  // ROS_INFO("point_num = %i", point_num); // added by Suqin He for debugging
...
void SensorProcessorBase::readcomputerparam(float *min_r, float *beam_a, float *beam_c, Eigen::RowVector3f *sensorJacobian, Eigen::Matrix3f *C_SB_transpose, Eigen::RowVector3f *P_mul_C_BM_transpose, Eigen::Matrix3f *B_r_BS_skew)
{
  ...
  // ROS_INFO("check5"); // added by Suqin He for debugging
	*beam_c = sensorParameters_.at("beam_constant");
  // ROS_INFO("check6"); // added by Suqin He for debugging
	*beam_a = sensorParameters_.at("beam_angle");
  // ROS_INFO("check7"); // added by Suqin He for debugging
}
